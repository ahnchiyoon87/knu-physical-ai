...
from .rules import classification_counts, first_threshold_crossing
...
def table_classifier(rows: list[dict], feature_ids: list[int], label_id: int, label_values: dict, train_fraction: float, split: str='ordered', seed: int=42):
    ...
    scores = classification_counts(y[test].astype(int).tolist(), predicted)
    return {'metrics': scores, 'train_rows': len(train), 'test_rows': len(test), 'split': split, 'features': feature_ids, 'predictions': [{'row_no': rows[int(i)]['row_no'], 'predicted': int(p), 'expected': int(y[i])} for i, p in zip(test, predicted)], 'limitations': '전처리는 학습 구간에서만 적합. 파일 순서는 실제 시각과 같다는 보장이 없음'}
...
def threshold_forecast(values: list[float], threshold: float, direction: str):
    ...
    forecast.update(threshold=threshold, direction=direction, median_crossing=first_threshold_crossing(forecast['median'], threshold, direction), lower_crossing=first_threshold_crossing(forecast['lower'], threshold, direction), upper_crossing=first_threshold_crossing(forecast['upper'], threshold, direction), interpretation='예측 구간 내 임계 도달 순서. 실제 고장까지 남은 수명 또는 시간으로 바꾸지 않음')
    return forecast
...
def cmapss_regression(train_path, test_path, labels_path, sensor_indices: list[int]):
    ...
    truth = pd.read_csv(labels_path, sep='\\s+', header=None)[0].to_numpy()
    ...
    return {'mae_cycles': float(mean_absolute_error(truth, predicted)), 'rmse_cycles': float(np.sqrt(mean_squared_error(truth, predicted))), 'units': len(last), 'predicted': predicted.tolist(), 'expected': truth.tolist(), 'provenance': 'NASA C-MAPSS 대체 도메인', 'claim': '제트 엔진 시뮬레이션의 사이클 기준 평가. 사출 RUL 검증 아님'}
