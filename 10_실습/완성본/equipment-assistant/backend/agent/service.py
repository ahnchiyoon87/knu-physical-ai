...
async def observe(state: AgentState):
    ...
    result = await gateway.tool("query_events", {"profile": state["profile"], "source_id": state["source_id"],
                                                "lot_id": state.get("lot_id"), "rid": state["rid"]})
    changed["events"] = result
    if result["status"] != "ok":
        changed["response"] = reply(state["rid"], status="insufficient" if result["status"] == "none" else result["status"],
                                    reason=result["reason"]).model_dump(mode="json")
    return changed


async def retrieve(state: AgentState):
    changed = step(state, "retrieve")
    result = await gateway.tool("search_docs", {"profile": state["profile"], "question": state["question"],
                                               "rid": state["rid"]})
    ...
    if result["status"] != "ok":
        ...
        changed['response']=reply(state['rid'],{'events':state['events']['answer'],'paths':state.get('paths'),
            'scope':'자율성 0: 근거 조회까지만 수행'},evidence=result['evidence']).model_dump(mode='json')
    return changed
...
async def relate(state: AgentState):
    ...
    if not state.get('lot_id') or not state.get('graph_end'):
        ...
        return changed
    result=await gateway.tool('graph_path',{'start':f"{state['profile']}:{state['source_id']}:lot:{state['lot_id']}",
                                           'end':state['graph_end'],'rid':state['rid']})
...
async def propose(state: AgentState):
    ...
    if rule is None:
        ...
        return changed
    candidate, meta = gateway.suggest(state["provider"], state["question"], state["events"]["answer"],
                                     state["documents"]["evidence"], rule,state.get('paths'))
    ...
    if candidate.get("propose") is not True:
        changed["response"] = reply(state["rid"], changed['draft_set'],status="insufficient",
            reason=candidate.get("rationale") or "변경 제안에 필요한 근거가 부족합니다", meta=meta).model_dump()
        return changed
    ...
    result = await gateway.tool("propose_action", {"rid": state["rid"], "rule_id": state["rule_id"],
        "event_ids": [rows[i]["id"] for i in indices], "new_width": candidate["new_width"],
...
async def await_approval(state: AgentState):
    # Interrupt nodes restart on resume. Only read state before interrupt; no repeated side effects.
    proposal = state["proposal"]["answer"]
    signal = interrupt({"proposal_id": proposal["id"], "version": proposal["version"],
    ...
    if stored["status"] == "rejected":
        changed["response"] = reply(state["rid"], {"proposal_id": stored["id"], "decision": "rejected"},
            status="refused", reason=stored["reason"]).model_dump()
    return changed
...
async def record(state: AgentState):
    ...
    proposal = state["proposal"]["answer"]
    result = await gateway.tool("record_decision", {"rid": state["rid"], "identity": proposal["id"],
        "version": proposal["version"], "payload_hash": proposal["payload_hash"]})
...
async def replay(state: AgentState):
    ...
    if result.status != "ok":
        changed["response"] = reply(state["rid"], {"decision": state["decision"]["answer"]},
            status=result.status, reason="결정은 기록됐지만 재생은 완료되지 않았습니다: " + result.reason).model_dump(mode="json")
        return changed
    changed["response"] = reply(state["rid"], {"decision": state["decision"]["answer"],
        "after": result.answer, "comparison_scope": "동일하게 저장한 입력의 규칙 재생",
        "quality_improved": "미판정"}, evidence=state["documents"]["evidence"],
        meta={"warnings": ["통지 수의 변화는 품질 개선의 증거가 아닙니다"]}).model_dump(mode="json")
    return changed


def graph(checkpointer):
    builder = StateGraph(AgentState)
    for name, node in [("observe", observe), ("relate",relate), ("retrieve", retrieve), ("propose", propose),
                       ("approval", await_approval), ("record", record), ("replay", replay)]:
        builder.add_node(name, node)
    builder.add_edge(START, "observe")
    for current, following in [("observe", "relate"), ("relate","retrieve"), ("retrieve", "propose"), ("propose", "approval"),
                               ("approval", "record"), ("record", "replay")]:
        builder.add_conditional_edges(current, lambda state: "stop" if state.get("response") else "next",
...
async def run(rid: str, inputs: dict, thread_id: str | None = None, resume_proposal: str | None = None):
    ...
    if result.get("__interrupt__"):
        return reply(rid, {"thread_id": thread_id, "proposal": result["proposal"]["answer"],"draft_set":result.get('draft_set')},
                     status="pending_approval", reason="사람의 결정이 저장된 뒤 이어집니다",meta=result.get('model_meta'))
    ...
    if result.get('model_meta'):response['meta']['agent_model']=result['model_meta']
    response["rid"] = rid
    return response


def route_question(rid: str, profile: str, source_ids: list[str], question: str, provider: str,
                   context: dict | None = None, graph_start: str | None = None, graph_end: str | None = None,
                   verify: bool = True, context_level: str = "probed"):
