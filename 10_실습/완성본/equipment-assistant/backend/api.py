...
from backend.agent import service as agent
...
from backend.eval import service as evaluation
...
from backend.process import service as process
from backend.rag import service as rag

app = FastAPI(title="설비 이상 대응 어시스턴트", version="0.1.0")
app.add_middleware(CORSMiddleware, allow_origins=[settings()["service"]["frontend"]],
                   allow_methods=["GET", "POST"], allow_headers=["Authorization","Content-Type","X-Request-ID","X-Approver-Token"])
...
def approve_role(x_approver_token: str = Header(default="")):
    if not secrets.compare_digest(x_approver_token, env("APPROVER_TOKEN")):
        from fastapi import HTTPException
        raise HTTPException(status_code=403, detail="사람 승인용 토큰이 필요합니다")
...
async def http_error(request, exc):
    return JSONResponse(status_code=exc.status_code, content=reply(request_id.get(), status="error",
                        reason=str(exc.detail)).model_dump())


@app.exception_handler(RequestValidationError)
async def invalid_request(request, exc):
    fields = [".".join(str(value) for value in error["loc"]) for error in exc.errors()]
    return JSONResponse(status_code=422, content=reply(request_id.get(), status="error",
                        reason="요청 항목을 확인하세요: " + ", ".join(fields)).model_dump())


@app.exception_handler(Exception)
async def failed(request, exc):
    emit("ui", "ui.request", "backend/api.py:failed", status="failed", reason=type(exc).__name__)
    return JSONResponse(status_code=500, content=reply(request_id.get(), status="failed",
                        reason=f"요청 실행 실패({type(exc).__name__}). 요청 번호로 관측 기록을 확인하세요").model_dump())
...
class AgentResume(Input):
    thread_id: str
    proposal_id: str


class Route(Query):
    graph_start: str | None = None
    graph_end: str | None = None


class Evaluation(Profile):
    provider: Literal["api","internal"]
    cases: list[dict]


class ModelJob(Source):
    method: Literal["table","pyod","forecast","threshold","rul","vision"]
    parameters: dict
...
@app.get("/health")
def health():
    checks = {"server":"ok", "database":"unconfigured", "model_calls_enabled":
              os.getenv("ALLOW_MODEL_CALLS", "false").lower() == "true"}
    ...
    return reply(request_id.get(), {"checks":checks,"day":settings()["service"]["day"],
                                   "routes":[route.path for route in app.routes if route.path.startswith("/api/")],
                                   "readiness":"서버 상태와 DB 연결만 확인. 모델·실습 완주 확인 아님"})
...
def vision_image(artifact_id: str,image_id: str,kind: Literal['original','heatmap']='original'):
    ...
    if path.suffix.lower() not in {'.png','.jpg','.jpeg'} or not path.is_file():raise ValueError('저장된 이미지 파일을 확인하세요')
...
@app.post("/api/eval/run", dependencies=[Depends(authorize)])
def evaluate(body: Evaluation):
    return evaluation.run(request_id.get(), **body.model_dump())


@app.get("/api/approvals", dependencies=[Depends(authorize)])
def approvals():
    return reply(request_id.get(), process_gateway.proposals())


@app.post("/api/process/propose", dependencies=[Depends(authorize)])
def propose_action(body: Proposal):
    return process.propose(request_id.get(), **body.model_dump())


@app.post("/api/approvals/{identity}/decide", dependencies=[Depends(authorize),Depends(approve_role)])
def decide_action(identity: str, body: Decision):
    return process.decide(request_id.get(), identity, **body.model_dump())


@app.post("/api/process/record", dependencies=[Depends(authorize)])
def record_action(body: Record):
    return process.record(request_id.get(), **body.model_dump())


@app.post("/api/agent/run", dependencies=[Depends(authorize)])
async def run_agent(body: AgentRun):
    return await agent.run(request_id.get(), body.model_dump())


@app.post("/api/agent/resume", dependencies=[Depends(authorize)])
async def resume_agent(body: AgentResume):
    return await agent.run(request_id.get(), {}, body.thread_id, body.proposal_id)


@app.post("/api/agent/route", dependencies=[Depends(authorize)])
def route_question(body: Route):
    return agent.route_question(request_id.get(), **body.model_dump())


# Register static files last, so API and trace paths retain their own contracts.
@app.get('/api/agent/drafts/{identity}',dependencies=[Depends(authorize)])
def agent_draft(identity:str,kind:str|None=None):
    from backend.agent.drafts import read
    return read(request_id.get(),identity,kind)


@app.get('/api/agent/drafts/{identity}/8d',dependencies=[Depends(authorize)])
def agent_eight_d(identity:str):
    from backend.agent.drafts import eight_d
    return eight_d(request_id.get(),identity)
