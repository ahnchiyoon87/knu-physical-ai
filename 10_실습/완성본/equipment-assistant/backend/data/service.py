...
def catalog(profile: str) -> list[dict]:
    ...
    return [{"source_id": key, "table": value["table"], "provenance": value["provenance"],
             "time_column": value.get("time_column"), "lot_rule": value.get("lot_rule"),
             "joins":value.get("joins",[]),
             "columns": value["columns"]}
            for key in mapping()["profiles"][profile]["sources"] for value in [analysis_source(key,profile)]]
...
def question(rid: str, profile: str, source_ids: list[str], text: str, provider: str,
             context: dict | None = None, verify: bool = True, context_level: str = "probed"):
    ...
    if context_level=="names":
        catalog_input=[{"source_id":entry["source_id"],"table":entry["table"],
            "columns":[{k:c[k] for k in ("id","name","type")} for c in entry["columns"]]} for entry in selected]
    ...
    checks = [{"item": "열 번호가 등록되어 있음", "result": "PASS"},
              {"item": "읽기 전용 SQL 계약", "result": "PASS"},
              {"item": "사람의 최종 검토", "result": "UNKNOWN"}]
    checks+=semantic
    answer = {**result, "sql": proposal["sql"], "checks": checks, "repair_history":history,
              "judgement_score":sum(check["result"]=="PASS" for check in semantic),
              "judgement_total":len(semantic),"probe":probes,
              "context": {"sql": proposal["sql"], "profile": profile, "source_ids": source_ids}}
    evidence = [{"kind": "table", "ref": ", ".join(source_ids), "sql": proposal["sql"],
                 "rows": len(result["rows"]), "provenance": [x["provenance"] for x in selected]}]
    if any(check["result"]!="PASS" for check in semantic):
        return reply(rid,{"sql":proposal["sql"],"checks":checks,"repair_history":history},status="failed",
                     reason="대상·범위·계산·의미의 확인이 끝나지 않았습니다. 답변으로 공개하지 않았습니다",meta=meta)
    meta["warnings"].append("모델 심사 통과는 실제 정답 보증이 아닙니다. 표와 질문을 대조하세요" if verify else "모델 의미 심사를 끈 비교 실행입니다. 사람이 직접 표와 질문을 대조하세요")
    return reply(rid, answer, status="ok" if result["rows"] else "none", evidence=evidence,
                 reason="" if result["rows"] else "조건에 맞는 행이 없습니다", meta=meta)
...
def series(rid: str, profile: str, source_id: str, column_id: int, lot_id: str | None):
    ...
    query = sql.SQL("SELECT row_no,lot_id,{} AS value FROM lab.{}{} ORDER BY row_no").format(
        sql.Identifier(f"c{column_id}"), sql.Identifier(config["table"]), condition).as_string()
    result = gateway.query_rows(query, {config["table"]})
    return reply(rid, {**result, "column": columns[column_id], "axis": "파일 행 순서",
                       "provenance": config["provenance"]},
                 status="ok" if result["rows"] else "none", reason="" if result["rows"] else "표시할 행 없음",
                 evidence=[{"kind": "table", "ref": source_id, "sql": query, "rows": len(result["rows"])}])
...
def quality(rid: str, profile: str, source_id: str):
    ...
    for column in numeric:
        ...
        for label, expression in [("missing", sql.SQL("count(*) FILTER (WHERE {} IS NULL)")),
                                  ("zero", sql.SQL("count(*) FILTER (WHERE {}=0)")),
                                  ("negative", sql.SQL("count(*) FILTER (WHERE {}<0)")),
                                  ("min", sql.SQL("min({})")), ("max", sql.SQL("max({})"))]:
            projections.append(sql.SQL("{} AS {}").format(expression.format(name),
                               sql.Identifier(f"{column['id']}_{label}")))
    statement = sql.SQL("SELECT {} FROM lab.{}").format(sql.SQL(",").join(projections),
                                                       sql.Identifier(config["table"])).as_string()
    result = gateway.query_rows(statement, {config["table"]})
    return reply(rid, {"statistics": result["rows"], "columns": numeric,
                       "note": "0·음수·최댓값은 관측값입니다. 삭제 사유나 고장 원인 판정이 아닙니다."},
                 evidence=[{"kind": "table", "ref": source_id, "sql": statement, "rows": 1}])
