    ...
    for samples in groups.values():
        ...
        for offset, row in enumerate(samples):
            ...
            if flagged:
                ...
                alarms.append({**row, "baseline_mean": mean, "baseline_std": std,
                               "suppressed": suppressed, "priority": definition.get("priority", "review"),
...
def detect_multichannel(rows, definition, counts=None):
    ...
    suppression=int(definition.get("suppression_shots",0))
    if minimum<2 or suppression<0:raise ValueError("기준 구간과 억제 간격을 확인하세요")
    groups={};alarms=[]
    for row in rows:groups.setdefault(row['lot_id'],[]).append(row)
    for samples in groups.values():
        samples=sorted(samples,key=lambda row:row['row_no'])
        if len({row['row_no'] for row in samples})!=len(samples):raise ValueError('순서 중복')
        if len(samples)<=minimum:raise ValueError('조합 감지의 기준 뒤 관측이 부족합니다')
        baselines={}
        for condition in conditions:
            cid=str(condition['column_id'])
            if condition['direction'] not in {'above','below'}:raise ValueError('조합 방향을 확인하세요')
            if finite_number(condition['percent'],'변화율')<=0:raise ValueError('변화율은 양수여야 합니다')
        ...
        for offset,row in enumerate(samples[minimum:],minimum):
            ...
            for condition in conditions:
                cid=str(condition['column_id']);value=finite_number(row['channels'][cid],'채널값')
            ...
            if (all(checks) if definition['operator']=='and' else any(checks)):
                ...
                alarms.append({**row,'conditions':details,'suppressed':suppressed,
                    'priority':definition.get('priority','review'),'rule_version':definition['version']})
    return alarms
...
def classification_counts(expected: list[int], predicted: list[int]) -> dict:
    ...
    return {"tp": tp, "tn": tn, "fp": fp, "fn": fn, "n": len(expected),
            "accuracy": (tp + tn) / len(expected), "precision": tp / (tp + fp) if tp + fp else None,
