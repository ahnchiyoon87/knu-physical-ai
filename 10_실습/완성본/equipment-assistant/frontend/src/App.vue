...
const panels=[['표와 질문',DataPanel],['품질',QualityPanel],['관계',GraphPanel],['감지',DetectPanel],['분석',ModelPanel],['리포트',ReportPanel],['문서 근거',RagPanel],['평가',EvalPanel],['에이전트',AgentPanel],['사람 승인',ApprovalPanel]]
...
<template>
 ...
 <section class="connection" v-if="settingsOpen"><label>서비스 접속 토큰<input type="password" v-model="session.token" autocomplete="off"/></label><label>자료<select v-model="session.profile"><option value="synthetic">교육용 합성</option><option value="kamp">KAMP 원본</option><option value="transfer">다른 주제</option></select></label><label>서비스 모델<select v-model="session.provider"><option value="api">API 모델</option><option value="internal">사내 모델</option></select></label><label>원천 ID<input v-model="session.source" list="sources"/><datalist id="sources"><option v-for="item in session.catalog" :key="item.source_id" :value="item.source_id"/></datalist></label><button :disabled="session.busy" @click="request('/health')">연결 확인</button></section>
