...
const sources=ref([]),verify=ref(true),level=ref('probed')
...
async function plot(){const r=await request('/api/data/series',{profile:session.profile,source_id:session.source,column_id:Number(column.value),lot_id:lot.value||null});if(r.status==='ok')series.value=r.answer;else series.value=null}
const plotData=computed(()=>{
 const raw=series.value?.rows||[], rows=raw.filter(x=>x.value!==null)
 if(!rows.length)return null
 const min=Math.min(...rows.map(x=>x.value)),max=Math.max(...rows.map(x=>x.value)),first=raw[0].row_no,last=raw.at(-1).row_no
 let points=[],previous=null;const segments=[]
 for(const row of raw){
   if(row.value===null||previous!==null&&row.lot_id!==previous){if(points.length)segments.push(points.join(' '));points=[]}
   if(row.value!==null)points.push(`${70+690*(row.row_no-first)/Math.max(1,last-first)},${200-160*(row.value-min)/Math.max(.000001,max-min)}`)
   previous=row.lot_id
 }
...
<template>
 ...
 <fieldset><legend>순서에 따른 변화</legend><div class="form-row"><label>수치 열<select v-model="column"><option v-for="c in numeric" :key="c.id" :value="c.id">{{c.meaning}} · {{c.unit}}</option></select></label><label>LOT 범위<input v-model="lot" placeholder="비우면 전체"/></label><button @click="plot" :disabled="session.busy||column===null">관측값 보기</button></div>
 <figure v-if="plotData"><svg viewBox="0 0 800 250" role="img" :aria-label="series.column.meaning+'의 순서별 변화'"><line x1="40" y1="200" x2="760" y2="200" stroke="#9ca9b8"/><polyline v-for="(segment,index) in plotData.segments" :key="index" :points="segment" fill="none" stroke="#126d73" stroke-width="2"/><text x="5" y="45">{{plotData.max.toFixed(2)}}</text><text x="5" y="200">{{plotData.min.toFixed(2)}}</text><text x="70" y="220">{{plotData.first}}</text><text x="720" y="220">{{plotData.last}}</text><text x="330" y="240">파일 행 순서</text></svg><figcaption>{{series.column.meaning}} ({{series.column.unit}}) · {{series.provenance}}. 세로축은 현재 표시 범위에 맞춥니다. LOT가 바뀌거나 값이 빠진 곳에서는 선을 끊습니다.</figcaption></figure>
 </fieldset></section>
</template>
