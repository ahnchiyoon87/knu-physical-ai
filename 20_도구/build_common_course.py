...
UNITS = [
 ...
 ('규칙과 시계열 변화', 'DH', '생산 조건에 맞는 규칙과 시간 구간을 사용해 변화를 찾습니다.', '변화를 찾는 기준을 배웠습니다. 다음에는 이미지와 수명처럼 입력과 목표가 다른 문제를 구별합니다.'),
 ('이미지 이상과 남은 수명', 'GI', '이미지의 이상 점수와 수명 예측이 각각 어떤 자료와 정답을 필요로 하는지 확인합니다.', '입력과 목표에 맞는 모델을 골랐으니 이제 반복 사용할 서비스 흐름에 연결하겠습니다.'),
 ('파이프라인과 모델 비교', 'EN', '적재부터 결과 저장까지 연결하고 단순 기준선과 모델의 차이를 같은 조건으로 평가합니다.', '수치 처리의 연결을 만들었습니다. 다음에는 문서와 관계를 답의 근거로 연결합니다.'),
