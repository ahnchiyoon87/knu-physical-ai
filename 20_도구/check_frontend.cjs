...
const manifest=common?JSON.parse(fs.readFileSync(path.join(root,'30_기록','공통코드_매니페스트.json'),'utf8')).records:JSON.parse(fs.readFileSync(path.join(root,'30_기록','출발본_매니페스트.json'),'utf8'))
const completed=path.join(root,'30_기록','완성본_매니페스트.json')
if(!common&&fs.existsSync(completed))manifest.push(...JSON.parse(fs.readFileSync(completed,'utf8')))
const folders=[...(common?[]:[app]),...manifest.filter(x=>x.features.length).map(x=>path.join(root,x.folder))]
...
fs.writeFileSync(path.join(root,'30_기록',common?'공통코드_Vue_구문검사.json':'Vue_구문검사.json'),JSON.stringify(result,null,2))
