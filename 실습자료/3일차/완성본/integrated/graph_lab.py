...
SOP_KEY_COOL_V2 = "SOP-COOL-001@v2"
...
OPTIONAL MATCH (e)-[:CITES]->(sec:SOPSection)<-[:HAS_SECTION]-(sop:SOP)
...
def sensors_with_recent_values(asset_id: str, run_id: str, seconds: int = 60) -> list[dict]:
    ...
    for s in run_read(Q_ASSET_SENSORS, asset_id=asset_id):
        rows = store.recent_window(asset_id, seconds, sensor_id=s["code"], run_id=run_id)
