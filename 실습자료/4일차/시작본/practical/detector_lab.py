"""S04 · 지속 조건·중복 억제·고정 시험 구간 (학생 시작본).
...
class SustainDetector:
    ...
    def feed(self, row: dict) -> list[dict]:
        ...
        # TODO(학생): 지금은 raw_value 로 센다. 급등(SPIKE)·고착(STUCK) 값도 '기준 초과'로 세어진다.
        #            품질 플래그가 OK 인 정제 값(value)만 세도록 고친다.
        ...
        if raw is not None and raw > self.threshold_c:
            ...
            # TODO(학생): 지금은 기준 초과가 아니면 무조건 초기화한다. 짧은 결측(MISSING) 1~2초나
            #            단발 급등(SPIKE) 때문에 지속 판단이 처음부터 다시 시작된다.
            #            초기화는 '유효 관측이 기준 이하(OK)' 또는 '긴 결측(GAP)'일 때만 한다.
            st.count = 0
        return []
...
class EventBook:
    ...
    def create(self, signal: dict) -> tuple[dict, bool]:
        # TODO(학생): 같은 설비·같은 유형의 열린 사건(open_event)이 있으면 새로 만들지 말고
        #            (기존 사건, False) 를 돌려준다. 억제한 횟수는 기존 사건의 "suppressed" 에 1 더한다.
        ev = {
...
def fixed_test_ids() -> list[int]:
    ...
    # TODO(학생): 부를 때마다 시험 사이클이 바뀐다. 오늘 결과와 다음 주 결과를 비교할 수 없다.
    #            제공 함수 hydops.b5_detect.evaluate.fixed_split() 이 고정한 시험 구간을 쓰도록 고친다.
    import numpy as np

    ds = E.load_reduced()
    rng = np.random.default_rng()
    return sorted(int(x) for x in rng.choice(ds.profile.index.to_numpy(), size=180, replace=False))
...
def compare_sustain(values=(1, 5, 10, 20), k_sigma: float = 4.0, min_delta_c: float = 3.0, smooth_s: int = 5) -> list[dict]:
    ...
    out = []
    for s in values:
        # TODO(학생): 위치 인자 순서가 evaluate(k_sigma, min_delta_c, sustain_s, smooth_s) 와 맞지 않는다.
        #            지금은 sustain_s 자리에 smooth_s 가, smooth_s 자리에 s 가 들어간다. 이름 인자로 고친다.
        r = E.evaluate(k_sigma, min_delta_c, smooth_s, s)
        out.append(
            {
                "sustain_s": s,
                "tp": r["tp"],
