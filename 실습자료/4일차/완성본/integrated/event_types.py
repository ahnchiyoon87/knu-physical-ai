...
def classify_sensor_window(ts1: dict, th=TH) -> str:
    ...
    if q["longest_gap_s"] >= 5 or q["flags"].get("STUCK", 0) > 0 or q["valid_ratio"] < 0.7:
        return SENSOR_FAULT
    # ② 센서가 정상일 때만: 유효 온도가 경보 온도를 넘은 채 지속 시간 이상 이어졌는가
    if ts1["longest_valid_above_alarm_s"] >= th.alarm_sustain_s:
        return COOLING_ANOMALY
    return NORMAL


def classify_lot(rows: list[dict]) -> dict | None:
    """한 로트의 합성 검사값 목록 → 규격 이탈이면 제품 품질 사건 요약, 아니면 None."""
    out = [r for r in rows if not (r["lsl"] <= r["value"] <= r["usl"])]
    if not out:
        return None
    return {
        "event_type": PRODUCT_QUALITY,
...
LABEL_KIND = {
    "profile.cooler_pct": "EQUIPMENT_STATE",  # UCI 냉각기 상태 라벨 (평가에만 쓴다)
    "inspection.bore_diameter": "PRODUCT_QUALITY",  # 합성 제품 검사값
    "observation.quality_flag": "SENSOR_QUALITY",  # B2 품질 플래그
}

# 제공 관제 화면(app.js TYPE_KO)과 같은 사건 유형 표시 이름
EVENT_DISPLAY = {
    "COOLING_ANOMALY": "냉각 이상",
    "SENSOR_FAULT": "센서 오류",
    "PRODUCT_QUALITY": "제품 품질(합성)",
}

# 사건 유형별로 사건이 흔히 끝나는 상태 (제공 화면 STATUS_KO 의 키)
TYPICAL_END_STATUS = {
    "SENSOR_FAULT": "SENSOR_CHECK",  # 설비 조치 보류, 센서 점검
    "PRODUCT_QUALITY": "HOLD_NO_EVIDENCE",  # 로트 보류 제안 — 품질 담당 확인
}
