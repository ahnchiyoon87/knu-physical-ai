...
def build_filter(asset_id: str, include_superseded: bool = False) -> dict:
    ...
    flt: dict = {"asset_scope": {"$like": f"|{asset_id}|"}}  # 경계 문자 | 까지 넣어야 HYD-1 이 HYD-10 에 섞이지 않는다
...
def confirm_with_graph(candidates: list[tuple[dict, str, float]], asset_id: str, event_type: str, k: int = 4, include_superseded: bool = False) -> list[dict]:
    ...
    applicable = applicable_keys(asset_id, event_type, include_superseded)
    hits, seen = [], set()
    for meta, text, score in candidates:
        if (meta["doc_id"], int(meta["version"])) not in applicable or meta["id"] in seen:
            continue
        seen.add(meta["id"])
...
def verify_citations_lab(p: Proposal, trace: list[dict]) -> tuple[Proposal, list[str]]:
    ...
    retrieved = {(h["doc_id"], int(h["version"]), str(h["section"])) for t in trace if t["tool"] == "search_sop" for h in t["result"]["hits"]}
