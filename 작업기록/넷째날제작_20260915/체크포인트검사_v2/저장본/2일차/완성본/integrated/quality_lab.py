...
# ---------------------------------------------------------------------------
RULES = QualityRules(
    gap_hold_s=5,  # 몇 초 이상 연속으로 값이 없으면 GAP(판정 보류)인가
    stuck_run=8,  # 같은 값이 몇 번째 반복될 때부터 STUCK(고착)인가
    spike_delta={"TS1": 8.0, "PS1": 60.0, "FS1": 15.0},  # 1초 사이 온도(TS1)가 몇 °C 넘게 뛰면 SPIKE 인가
)
...
WHERE run_id = %(run_id)s
  ...
  AND ts > (SELECT max(ts) FROM observation
            WHERE run_id = %(run_id)s AND asset_id = %(asset_id)s) - interval '60 seconds'
ORDER BY ts
"""
...
# ---------------------------------------------------------------------------
NORMAL_RANGE = (0, 17)  # classify_window 결과가 VALID 인 구간
HOLD_RANGE = (26, 35)  # classify_window 결과가 SENSOR_FAULT(판정 보류)인 구간
