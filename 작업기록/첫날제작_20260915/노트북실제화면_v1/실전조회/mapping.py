"""실전반 S01 · 세 센서 CSV → 공통 관측 레코드 → PostgreSQL → 최근 60초 조회 (정답본).
...
def reduce_to_seconds(samples: np.ndarray, hz: int) -> list[dict]:
    ...
    return [
        {"mean": float(sec.mean()), "min": float(sec.min()), "max": float(sec.max()), "n": int(sec.size)}
        for sec in shaped
    ]
            ...
            rows.append(
                {
                    ...
                    "agg": None
                    if hz == 1
                    else {"min": round(st["min"], 4), "max": round(st["max"], 4), "n": st["n"], "source_hz": hz},
                    "is_synthetic": False,
                }
            )
    return rows
