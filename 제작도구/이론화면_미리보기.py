...
def content(lines):
    ...
    for line in lines:
        if line.startswith('|'):
            ...
            flush()
            result.append('<p class="question">'+inline(line)+'</p>')
...
section{width:1600px;height:900px;padding:70px 86px;background:#faf9f5;overflow:hidden;position:relative;break-after:page;flex-direction:column}
section>*{flex-shrink:0;width:100%}
h1{font-size:62px;line-height:1.2;margin:0 0 40px;font-weight:700;letter-spacing:-1.8px;color:#133b43}
main{font-size:32px;line-height:1.55}p{margin:0 0 27px;word-break:keep-all}main>:last-child{margin-bottom:0}strong{color:#007c78}
table{border-collapse:collapse;width:100%;font-size:30px;line-height:1.48;margin:0 0 30px;table-layout:fixed}
th{text-align:left;background:#133b43;color:white;font-weight:600;padding:16px 24px}
td{padding:16px 24px;border-bottom:2px solid #cedad8;vertical-align:top;word-break:keep-all}
code{font-family:Consolas,"Malgun Gothic",monospace;font-size:.92em}
#S01 table{border-collapse:separate;border-spacing:18px 30px;margin:0 -18px 28px;width:calc(100% + 36px)}
#S01 th,#S01 td{background:#133b43;color:#fff;border:0;border-radius:12px;text-align:center;vertical-align:middle;height:145px;font-size:32px;font-weight:600;padding:20px 10px}
