...
P = os.environ.get("BOOTSTRAP_PLATFORM_URL", "http://localhost:8910")
HYDOPS = os.environ.get("BOOTSTRAP_HYDOPS_URL", "http://localhost:8800")
PUBLIC_PLATFORM = os.environ.get("PUBLIC_PLATFORM_URL", "http://localhost:8910")
PUBLIC_HYDOPS = os.environ.get("PUBLIC_HYDOPS_URL", "http://localhost:8800")
...
def main():
    ...
    if "datasource" in stages:
        ...
        with psycopg.connect(os.environ.get("HYDOPS_PG_DSN", "postgresql://hydops:hydops@localhost:55432/hydops"), autocommit=True) as pg:
        ...
        print("datasource", ok(c.post(f"{P}/fabric/datasources", json={"name": "hydops_edu", "engine": "postgres", "description": "교육용 유압설비 운영 DB (읽기 전용 계정)", "parameters": {"host": os.environ.get("FABRIC_PG_HOST", "localhost"), "port": int(os.environ.get("FABRIC_PG_PORT", "55432")), "database": "hydops", "user": "lab_reader", "password": "lab_reader"}}))["server_version"])
    ...
    if "agent" in stages:
        definition = json.loads((T / "agent_ops.json").read_text(encoding="utf-8"))
        definition["model"] = os.environ.get("HYDOPS_LLM_MODEL", definition["model"])
        print("agent", ok(c.post(f"{P}/agents", json=definition))["agent_id"])
    ...
    if "app" in stages:
        app = ok(c.post(f"{P}/apps", json={"name": "hydops-ops", "template": "ops-console", "config": {"hydops_api": PUBLIC_HYDOPS, "platform_api": PUBLIC_PLATFORM, "schema_name": "HydraulicOps", "process_def_id": "cooling_response"}}))
