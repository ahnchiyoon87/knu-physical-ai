...
def publish(body: dict = Body(...)):
    ...
    for k in ("hydops_api", "platform_api"):
        try:
            internal_key = "HYDOPS_API" if k == "hydops_api" else "PLATFORM_INTERNAL_URL"
            base = os.environ.get(internal_key, cfg[k])
            r = httpx.get(base.rstrip("/") + ("/api/state" if k == "hydops_api" else "/health"), timeout=5)
